...
Opening is when an image is eroded and then the resulting image is then dilated using a specified kernel size
...
def run_open(img):
    if not img is None:  # if no image is passed, no process is taken place and an error is printed to the command line
        new_image = erode.erode(img)  # runs erosion on the input image and allocates it to new_image
        final_image = dilate.dilate(new_image)  # runs erosion on the new_image and allocates it to itself
...
if __name__ == "__main__":  # avoided when an instance is created in other classes, only runs when file is run
    ...
    cv2.imwrite(output_image, run_open(image))  # calls the dilation function on image and writes it to a file with
    # output_image name
